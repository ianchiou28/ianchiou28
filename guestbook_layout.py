...
text = re.sub(r'<a href="javascript:openGuestbook\(\)"[^>]*>留言板</a>', '', text)
...
guestbook_html = '''    <!-- Guestbook Section -->
    <section id="guestbook" class="py-20 px-4">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 glass-arcade rounded-3xl p-8">
                <!-- Submit Form -->
                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="score-message-input" maxlength="40" placeholder="Leave a message for all to see... (Optional)" class="w-full bg-[#1a2332] border-2 border-neon-purple/50 text-[#fdfaf5] px-4 py-3 font-mono focus:outline-none focus:border-neon-purple focus:shadow-[0_0_15px_rgba(122,118,255,0.4)] transition-all">
                        </div>
                ...
                <div>
                    ...
                    <div id="high-scores-board" class="space-y-3 h-[400px] overflow-y-auto pr-2 custom-scrollbar">
...
# We need to remove the openGuestbook() logic since the form is always visible on the page now.
# But we need to make sure the submit button logic runs on DOM load.
init_logic = '''
...
# Remove the old openGuestbook and handleSubmit logic entirely.
js = re.sub(r'function openGuestbook\(\) \{ let score = \"-\"; let gameName = \"GUEST\";[\s\S]*?\}', init_logic, js)
